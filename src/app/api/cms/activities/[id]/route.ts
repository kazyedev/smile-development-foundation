...
export async function GET(_req: Request, { params }: Params) {
  ...
  if (error) {
    console.error("GET /api/cms/activities/[id] - Database error:", error);
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  if (authError || !user) {
    console.error("PATCH /api/cms/activities/[id] - Authentication failed:", authError);
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const payload = await req.json().catch(() => ({}));
  console.log("PATCH /api/cms/activities/[id] - Payload:", JSON.stringify(payload, null, 2));
  ...
  console.log("PATCH /api/cms/activities/[id] - Database payload:", JSON.stringify(databasePayload, null, 2));
  ...
  if (error) {
    console.error("PATCH /api/cms/activities/[id] - Database error:", error);
    if (error.code === "PGRST116") {
      return NextResponse.json({ error: "Activity not found" }, { status: 404 });
    }
    return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const { data: { user }, error: authError } = await supabase.auth.getUser();
  if (authError || !user) {
    console.error("DELETE /api/cms/activities/[id] - Authentication failed:", authError);
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const { error } = await supabase.from("activities").delete().eq("id", id);
  if (error) {
    console.error("DELETE /api/cms/activities/[id] - Database error:", error);
    return NextResponse.json({ error: error.message }, { status: 400 });
  }
