...
const transformToDatabaseFields = (data: any) => {
  ...
  if (data.alt !== undefined) transformed.alt = data.alt;
  ...
  if (data.takenAt !== undefined) transformed.taken_at = data.takenAt;
  if (data.photographer !== undefined) transformed.photographer = data.photographer;
...
export async function GET(_req: Request, { params }: Params) {
  ...
  if (error) {
    console.error("GET /api/cms/images/[id] - Database error:", error);
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  if (authError || !user) {
    console.error("PATCH /api/cms/images/[id] - Authentication failed:", authError);
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const payload = await req.json().catch(() => ({}));
  console.log("PATCH /api/cms/images/[id] - Payload:", JSON.stringify(payload, null, 2));
  ...
  console.log("PATCH /api/cms/images/[id] - Database payload:", JSON.stringify(databasePayload, null, 2));
  ...
  if (error) {
    console.error("PATCH /api/cms/images/[id] - Database error:", error);
    if (error.code === "PGRST116") {
      return NextResponse.json({ error: "Image not found" }, { status: 404 });
    }
    return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const { data: { user }, error: authError } = await supabase.auth.getUser();
  if (authError || !user) {
    console.error("DELETE /api/cms/images/[id] - Authentication failed:", authError);
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const { error } = await supabase.from("images").delete().eq("id", id);
  if (error) {
    console.error("DELETE /api/cms/images/[id] - Database error:", error);
    return NextResponse.json({ error: error.message }, { status: 400 });
  }
