...
const transformToDatabaseFields = (data: any) => {
  ...
  if (data.videoUrl !== undefined) transformed.video_url = data.videoUrl;
  ...
  if (data.banners !== undefined) transformed.banners = data.banners;
  ...
  if (data.deliverables !== undefined) transformed.deliverables = data.deliverables;
  if (data.resources !== undefined) transformed.resources = data.resources;
  if (data.cost !== undefined) transformed.cost = data.cost;
  if (data.beneficiaries !== undefined) transformed.beneficiaries = data.beneficiaries;
  ...
  if (data.activityIds !== undefined) transformed.activity_ids = data.activityIds;
...
export async function GET(_req: Request, { params }: Params) {
  ...
  if (error) {
    console.error("GET /api/cms/projects/[id] - Database error:", error);
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  if (authError || !user) {
    console.error("PATCH /api/cms/projects/[id] - Authentication failed:", authError);
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const payload = await req.json().catch(() => ({}));
  console.log("PATCH /api/cms/projects/[id] - Payload:", JSON.stringify(payload, null, 2));
  ...
  console.log("PATCH /api/cms/projects/[id] - Database payload:", JSON.stringify(databasePayload, null, 2));
  ...
  if (error) {
    console.error("PATCH /api/cms/projects/[id] - Database error:", error);
    if (error.code === "PGRST116") {
      return NextResponse.json({ error: "Project not found" }, { status: 404 });
    }
    return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const { data: { user }, error: authError } = await supabase.auth.getUser();
  if (authError || !user) {
    console.error("DELETE /api/cms/projects/[id] - Authentication failed:", authError);
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const { error } = await supabase.from("projects").delete().eq("id", id);
  if (error) {
    console.error("DELETE /api/cms/projects/[id] - Database error:", error);
    return NextResponse.json({ error: error.message }, { status: 400 });
  }
