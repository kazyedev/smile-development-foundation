...
import { insertVideoSchema } from "@/lib/db/schema/videos";
...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    .from("videos")
    .select(
      "id, title_en, title_ar, is_published, views, created_at, mime_type, size, width, height, url"
    )
  ...
  if (error) {
    console.error("GET /api/cms/videos - Database error:", error);
...
export async function POST(request: NextRequest) {
  try {
    console.log("POST /api/cms/videos - Starting request");
    const supabase = await supabaseServer();
    ...
    // Validate the input
    const validatedData = insertVideoSchema.parse(body);
