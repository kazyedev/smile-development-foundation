...
interface MediaCategoryListItem {
  id: number;
  name_en: string;
  name_ar: string;
...
export default function MediaCategoriesPage() {
  ...
  const [categories, setCategories] = useState<MediaCategoryListItem[]>([]);
  const [selectedIds, setSelectedIds] = useState<Set<number>>(new Set());
  const [loading, setLoading] = useState(true);
  const [deleteId, setDeleteId] = useState<number | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [sortBy, setSortBy] = useState<"name" | "page_views" | "created_at">("created_at");
  ...
  const t = {
    ...
    ar: {
      title: "فئات الوسائط",
      description: "إدارة فئات الوسائط وتصنيفات المحتوى",
      sort: "ترتيب",
      refresh: "تحديث",
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/cms/media-categories${qs}`);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/cms/media-categories/bulk-delete", {
        method: "POST",
        headers: {
  ...
  const handleSort = (column: "name" | "page_views" | "created_at") => {
    if (sortBy === column) {
  ...
  const sortedCategories = [...categories].sort((a, b) => {
    ...
    if (sortBy === "name") {
      aVal = isArabic ? a.name_ar : a.name_en;
      bVal = isArabic ? b.name_ar : b.name_en;
    ...
    <div className="space-y-6" dir={isArabic ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/${locale}/cms/media/categories/new`}>
            <Button
              size="sm"
              className="flex items-center gap-2"
            >
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              >
                {isArabic ? text.nameAr : text.nameEn}
              </TableHead>
            ...
            {loading ? (
              ...
              sortedCategories.map((category) => (
                <TableRow key={category.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Link href={`/${locale}/cms/media/categories/${category.id}/edit`}>
                        <Button
                          variant="ghost"
                          size="sm"
