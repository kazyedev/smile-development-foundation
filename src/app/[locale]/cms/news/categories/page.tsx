...
interface NewsCategoryListItem {
  id: number;
  name_en: string;
  name_ar: string;
...
export default function NewsCategoriesPage() {
  ...
  const [categories, setCategories] = useState<NewsCategoryListItem[]>([]);
  ...
  const t = {
    ...
    ar: {
      title: "فئات الأخبار",
      description: "إدارة فئات الأخبار وتصنيفات المواضيع",
      sort: "ترتيب",
      refresh: "تحديث",
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/cms/news-categories${qs}`);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/cms/news-categories/bulk-delete", {
        method: "POST",
        headers: {
    ...
    <div className="space-y-6" dir={isArabic ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/${locale}/cms/news/categories/new`}>
            <Button
              size="sm"
              className="flex items-center gap-2"
            >
            ...
            {loading ? (
              ...
              sortedCategories.map((category) => (
                <TableRow key={category.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Link href={`/${locale}/cms/news/categories/${category.id}/edit`}>
                        <Button
                          variant="ghost"
                          size="sm"
