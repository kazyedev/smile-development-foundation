...
interface ActivityTagsFieldsProps {
  form: UseFormReturn<any>;
}

export function ActivityTagsFields({ form }: ActivityTagsFieldsProps) {
  ...
  const text = {
    ...
    description: isArabic ? "إدارة الكلمات المفتاحية والعلامات للنشاط" : "Manage keywords and tags for the activity",
    ...
    otherImagesTitle: isArabic ? "الصور الأخرى" : "Other Images",
    otherImagesDescription: isArabic ? "إضافة روابط صور أخرى للنشاط" : "Add other image URLs for the activity",
    imageUrl: isArabic ? "رابط الصورة" : "Image URL",
    imageUrlPlaceholder: isArabic ? "https://example.com/image.jpg" : "https://example.com/image.jpg",
    noImages: isArabic ? "لا توجد صور أخرى" : "No other images added"
  };
  ...
  const otherImagesUrl = watch("otherImagesUrl") || [];
  ...
  // Image URL handlers
  const addImageUrl = () => {
    if (newImageUrl.trim() && !otherImagesUrl.includes(newImageUrl.trim())) {
      setValue("otherImagesUrl", [...otherImagesUrl, newImageUrl.trim()]);
      setNewImageUrl("");
    }
  };

  const removeImageUrl = (url: string) => {
    setValue("otherImagesUrl", otherImagesUrl.filter((img: string) => img !== url));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={isArabic ? "text-right" : ""}>
        <CardHeader>
          <CardTitle>{text.otherImagesTitle}</CardTitle>
          <CardDescription>{text.otherImagesDescription}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Input
              ...
              onKeyDown={(e) => handleKeyDown(e, addImageUrl)}
              dir="ltr"
              type="url"
            />
            <Button
          ...
          <div className="space-y-2 min-h-[40px]">
            {otherImagesUrl.length === 0 ? (
              ...
              otherImagesUrl.map((url: string, index: number) => (
                <div key={index} className="flex items-center justify-between p-2 border rounded-md">
                  <span className="text-sm truncate flex-1 mr-2" dir="ltr">
                    {url}
                  </span>
                  <Button
                    type="button"
